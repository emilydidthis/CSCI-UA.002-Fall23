...
* (1 point) Reads and extracts data from both .txt files
* (1 point) Accurately checks to see if the course exists, counts the number of students correctly, prints the names of the students with proper formatting
...
Total points: 6.5 points
* (.5 points) Part 2a: `valid_username` and `valid_password` pass all tests
* (1 point) Part 2b: `username_exists` and `check_password` pass all tests
* (.5 points) Part 2c: `add_user` passes all tests, user_info.txt is properly updated
* (1.5 points) Part 2d: `send_message` passes all tests, message files added
* (1.25 points) Part 2e: `print_messages` and `delete_messages` work accordingly
* (1.75 points) Part 2f: program works and matches output
